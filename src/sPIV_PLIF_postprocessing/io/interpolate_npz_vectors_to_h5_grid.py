...
of frames as the u/v data files. Source and target grids may be 1D axes or 2D
mesh grids.
"""
...
def interpolate_velocity_npz_to_grid(
    ...
    """
    Interpolate u/v frame lists to the target HDF5 grid and save .npy stacks.

    Saved stacks have shape (ny, nx, n_frames).
    """
    u_frames = load_npz_frames(u_npz_path)
    v_frames = load_npz_frames(v_npz_path)
    x_frames = load_npz_frames(xgrid_npz_path)
    y_frames = load_npz_frames(ygrid_npz_path)
    frame_count = validate_frame_counts(
        (("u", u_frames), ("v", v_frames), ("xgrid", x_frames), ("ygrid", y_frames))
    )
    ...
    u_out = np.empty(output_shape, dtype=np.float32)
    v_out = np.empty(output_shape, dtype=np.float32)

    for frame_idx, (u_frame, v_frame, x_grid, y_grid) in enumerate(
        zip(u_frames, v_frames, x_frames, y_frames, strict=True)
    ):
        y_grid += 150
        logger.info("Interpolating frame %s/%s", frame_idx + 1, frame_count)
        u_out[:, :, frame_idx] = interpolate_frame(
            u_frame, x_grid, y_grid, target_x, target_y, method=method, fill_value=fill_value
        )
        v_out[:, :, frame_idx] = interpolate_frame(
            v_frame, x_grid, y_grid, target_x, target_y, method=method, fill_value=fill_value
        )

    output_dir.mkdir(parents=True, exist_ok=True)
    u_output_path = output_dir / f"{output_prefix}u.npy"
    v_output_path = output_dir / f"{output_prefix}v.npy"
    np.save(u_output_path, u_out)
    np.save(v_output_path, v_out)

    logger.info("Saved interpolated u stack to %s", u_output_path)
    logger.info("Saved interpolated v stack to %s", v_output_path)
    return u_output_path, v_output_path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Interpolate variable-size u/v .npz frame lists onto x/y grids from an HDF5 file."
    )
    ...
    parser.add_argument("--u-npz", type=Path, required=True, help=".npz file containing u frames.")
    parser.add_argument("--v-npz", type=Path, required=True, help=".npz file containing v frames.")
    ...
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory for saved u.npy/v.npy files.")
...
def main() -> None:
    ...
    interpolate_velocity_npz_to_grid(
        u_npz_path=args.u_npz,
        v_npz_path=args.v_npz,
